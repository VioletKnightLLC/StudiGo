...
pub trait IAMVideoProcAmp {
    ...
    ) -> Result<(i32, i32, i32, i32), VirtualCamError>;
...
pub trait IKsPropertySet {
    /// Set a property
    fn set(&self, guid: &str, prop_id: u32, data: &[u8]) -> Result<(), VirtualCamError>;

    /// Get a property
    fn get(&self, guid: &str, prop_id: u32, data: &mut [u8]) -> Result<usize, VirtualCamError>;

    /// Query if a property is supported
    fn query_supported(&self, guid: &str, prop_id: u32) -> Result<bool, VirtualCamError>;
}
...
impl VirtualCamFilter {
    ...
    pub fn set_format(&self, format: VideoFormat) -> Result<(), VirtualCamError> {
        if format.width == 0 || format.height == 0 {
            return Err(VirtualCamError::InvalidFormat(
    ...
    pub fn iam_stream_config(&self) -> VirtualCamIAMStreamConfig<'_> {
        VirtualCamIAMStreamConfig { filter: self }
    }

    // IAMVideoProcAmp implementation
    pub fn iam_video_proc_amp(&self) -> VirtualCamIAMVideoProcAmp<'_> {
        VirtualCamIAMVideoProcAmp { filter: self }
    }

    // IKsPropertySet implementation
    pub fn iks_property_set(&self) -> VirtualCamIKsPropertySet<'_> {
        VirtualCamIKsPropertySet { filter: self }
    }
}
...
impl<'a> IAMVideoProcAmp for VirtualCamIAMVideoProcAmp<'a> {
    fn get_range(
        ...
        match property {
            ...
            | VideoProcAmpProperty::Saturation => Ok((0, 255, 128, 1)),
            VideoProcAmpProperty::Hue => Ok((-180, 180, 0, 1)),
            VideoProcAmpProperty::Sharpness | VideoProcAmpProperty::Gamma => Ok((0, 100, 50, 1)),
            _ => Ok((0, 100, 50, 1)),
        }
    }
...
pub struct VirtualCamIKsPropertySet<'a> {
    // Retains the filter reference; IKsPropertySet currently needs no state access.
    #[allow(dead_code)]
    filter: &'a VirtualCamFilter,
}
