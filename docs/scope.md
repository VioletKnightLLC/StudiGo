...
|| **T1-WiFi** | **WiFi streaming support for GoPro (RTSP/UDP)** — Add GoPro WiFi streaming source. Connect to GoPro's built-in AP (192.168.x.x), use Open GoPro HTTP API to get RTSP URL, then stream via RTSP. Supports Hero 8+ WiFi mode. Reference: Open GoPro HTTP API spec. | cc | T1 | `cargo test wifi_source::tests` connects to a stub RTSP server and receives ≥ 60 frames. |
